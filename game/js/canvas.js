...
function buildGameCanvas() {
    ...
    buttonSphereStart = new createjs.Bitmap(
        loader.getResult("buttonSphereStart")
    );
    ...
    itemNumberSelectBg = new createjs.Bitmap(
        loader.getResult("itemNumberSelectBg")
    );
    ...
    for (var n = 0; n < score_arr.length + extraBall; n++) {
        $.prize["bg" + n] = new createjs.Bitmap(
            loader.getResult("itemPrizeBg")
        );
        centerReg($.prize["bg" + n]);
        $.prize["bg" + n].x = startX;
        $.prize["bg" + n].y = startY;

        $.prize["bgselect" + n] = new createjs.Bitmap(
            loader.getResult("itemPrizeSelectBg")
        );
        ...
        if (bonusBall) {
            if (n == 0) {
                ...
                scoreString = scoreTextDisplay.replace(
                    "[NUMBER]",
                    addCommas(score_arr[totalNum - 1].prize)
                );
    ...
    resultTitleTxt.font = "bold 50px roboto";
    resultTitleTxt.color = "#ddb867";
    resultTitleTxt.textAlign = "center";
    resultTitleTxt.textBaseline = "alphabetic";
    resultTitleTxt.text = "Chúc mừng bạn";
    resultTitleTxt.x = canvasW / 2;
    resultTitleTxt.y = (canvasH / 100) * 45;

    resultShareTxt = new createjs.Text();
    resultShareTxt.font = " bold 30px roboto";
    ...
    buttonContinue.x = canvasW / 2 + 150;
    buttonContinue.y = (canvasH / 100) * 70;

    // tạo button huỷ

    buttonCancelResult = new createjs.Bitmap(loader.getResult("buttonCancel"));
    centerReg(buttonCancelResult);
    createHitarea(buttonCancelResult);
    buttonCancelResult.x = canvasW / 2 - 150;
    buttonCancelResult.y = (canvasH / 100) * 70;

    //option
    buttonFullscreen = new createjs.Bitmap(
        loader.getResult("buttonFullscreen")
    );
    ...
    gameContainer.addChild(
        ...
        itemBar,
        itemBarBonus,
        ballsRevealContainer
        // itemCard,
        // itemBarUser,
        // ballsSelectContainer,
        // buttonLucky,
        // cardContainer,
        // tableContainer,
        // selectTitleTxt
